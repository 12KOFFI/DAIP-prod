...
class Filesystem
{
    ...
    // start time for long operations (used to avoid silent PHP timeout)
    private static ?float $operationStartTime = null;
    private const MAX_OPERATION_SECONDS = 30; // adjust lower/higher during debugging
    ...
    {
        // initialize operation start time once per top-level call
        if (null === self::$operationStartTime) {
            self::$operationStartTime = microtime(true);
        }

        // check for timeout guard
        $checkTimeout = function () {
            if (null !== self::$operationStartTime && (microtime(true) - self::$operationStartTime) > self::MAX_OPERATION_SECONDS) {
                // reset for next operations
                self::$operationStartTime = null;
                throw new IOException(sprintf('Filesystem operation timed out after %.2f seconds.', self::MAX_OPERATION_SECONDS));
            }
        };
        ...
        // reset start time when top-level call completes
    ...
    {
        // initialize operation start time for mirror
        if (null === self::$operationStartTime) {
            self::$operationStartTime = microtime(true);
        }
        $checkTimeout = function () {
            if (null !== self::$operationStartTime && (microtime(true) - self::$operationStartTime) > self::MAX_OPERATION_SECONDS) {
                self::$operationStartTime = null;
                throw new IOException(sprintf('Filesystem mirror operation timed out after %.2f seconds.', self::MAX_OPERATION_SECONDS));
            }
        };
        ...
        // reset start time after mirror completes
        self::$operationStartTime = null;
    }
